...
patientApp.post('/', zValidator('json', PatientSchema), (c) => {
    const data = c.req.valid('json');
    const stmt = db.prepare(`
      INSERT INTO Patient (Name, DOB, Gender, Phone, Address)
      VALUES (?, ?, ?, ?, ?)
    `);
    const result = stmt.run(data.Name, data.DOB ?? null, data.Gender ?? null, data.Phone ?? null, data.Address ?? null);
    const newPatient = {
...
patientApp.put('/:id', zValidator('json', UpdatePatientSchema), (c) => {
    const id = parseInt(c.req.param('id'));
    const data = c.req.valid('json');
    ...
    const updatedPatient = fetchStmt.get(id);
